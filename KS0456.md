...
| 2        | Keyestudio Edge Connector IO Breakout Board for micro:bit | 1       | ![](media/9c7d340c158c65aba57271f6d23d2c63.jpeg)   |
| 3        | Keyestudio micro:bit 1W LED Module                        | 1       | ![](media/821078e2d0d4bf98d971d6e060a18585.jpeg) |
| 4        | Keyestudio micro:bit 5050 RGB Module                      | 1       | ![](media/146798cc5d9b290c55e4ea52b0b74a3a.jpeg) |
| 5        | Keyestudio micro:bit Tactile Button Module                | 1       | ![](media/c8850ddabced9a2d4d72f62230b1deb1.jpeg) |
| 6        | Keyestudio micro:bit Capacitive Touch Module              | 1       | ![](media/8300551b2e4c1f87ec98f846efa2cbbe.jpeg) |
| 7        | Keyestudio micro:bit TEMT6000 Light Module                | 1       | ![](media/ef9196cf2bab0b8374ef29f0dee3c6a8.jpeg) |
| 8        | Keyestudio micro:bit PIR Motion Module                    | 1       | ![](media/d0ccdf4c80f4e7eb73a366f90282382b.jpeg) |
| 9        | Keyestudio micro:bit Microphone Module                    | 1       | ![](media/f95a7ff18513bcea0f174244070f1083.jpeg) |
| 10       | Keyestudio micro:bit Passive Buzzer Module                | 1       | ![](media/3995fbf0d964d278ce6b5ce77145a230.jpeg) |
| 11       | Black USB cable 1m                                        | 1       | ![](media/5c91e25363b158fa7e3f9bc957304c60.jpeg) |
| 12       | Alligator clip cable                                      | 10      | ![](media/236bd566a9b20f8e0517d01bcb0c8e07.jpeg)         |
...
**![](media/df4998f32388d7e4f5ed39dadce17119.jpeg)**When three signal
...
Go to the **Pins**, drag and drop the **analog write pin(P0) to (1023)** block
...
You can click on **JavaScript** and it will display JavaScript code there in
IDE.
...
**![](media/f8adbf9324b240210379b8dd39a2956a.png)**
...
Let's get started and display the analog value of ambient light on our
...
![](media/9883f31354c206852919e1845390d4be.jpeg)
...
And then again go to **Basic** and drag the **show leds** block beneath the
logic block you just made. Otherwise show another icon.

You can draw an image on the LED screen as you wish.

**![](media/77f976e5ea55d24061f266bbb3ab2a3c.png)**

After completing this step micro:bit will restart itself and run the code. It
will display icons on the simulator. Quite easy? Yeah, quite easy. So let's move
on and name and download the program we’ve written.
...
When detecting someone moving nearby, the LED matrix on the micro:bit main board
will show a heart shape icon, and led on the PIR module lights.
...
This keyestudio microphone module is fully compatible with micro:bit control
board. It is a sound detection device, an analog signal output device.

It is mainly composed of a MIC head, a LM358D chip and a potentiometer.

**![](media/ae0986019f5e6c667cd209c281a5f90e.jpeg)**When the
microphone detects sound, and converts it into a voltage signal; then amplifies
it through the LM358D chip.
...
![](media/c24a8fe8646ec6459c216b6b45a214d1.jpeg)**Technical Parameters**
...
Connect the keyestudio Microphone Module to micro:bit main board with 3
Alligator clip cables. Ring S to P0, V to 3V, G to GND.
...
So now let's move to coding. Let us see how to code and display the analog value
of microphone sound. Below are some steps to follow.
...
Let's get started and display the analog value of microphone sound on micro:bit.
To do so, you just need to go to **Basic** and scroll down to see an **on
start** block.
...
In this way it can write the sound value to the serial port and show it on
monitor.

![](media/289be0ae6122b49e6e791acbf5d4a262.png)

Change the “**x**” to **noise** and duplicate the block **analog read pin(P0)**
to replace the “**0**” field.
...
**![](media/2372f50e66cbc38de7e7e3504ec331eb.png)**
...
You should see the LED matrix show the scrolling digit.

Read the analog value of sound through the
software![](media/f113707beadd9b5b2929315bf915541b.png).

The louder the sound, the greater the analog value is.

![](media/d015625647bdb7d3df609a057abf8372.jpeg)

![](media/c8c2ad78a9c990dcfef0a96acaf3e64a.png)

Note: the baud rate of micro:bit is defaulted by 115200.

Pay close attention that you can turn the potentiometer on the module to adjust
the sound magnification, which also will enlarge the analog value.

![](media/10b7216c0089f328b9ef44e07f1eb4ba.png)
...
**![](media/3fcfefb5cedc3b065fe33c3ace6db5e4.jpeg)**When using, connect
the module to micro:bit control board using Crocodile clip line.
...
-   ![](media/78ce8a388cc17b7a057b5fbcfd54ca42.jpeg)Micro:bit main board
    \*1
...
Connect the keyestudio Passive Buzzer Module to micro:bit main board with 3
Alligator clip cables. Ring S to P0, V to 3V, G to GND.

**![](media/16ebfe3f72089ad7b37e3253dbabec58.png)**

**Coding**

So now let's move to coding. Let us see how we can code the buzzer play tune.
Below are some steps to follow.
...
Let's get started and code the buzzer play a tone. To do so, you just need to go
to **Basic** and scroll down to see an **on start** block.
...
Now go to the **Basic** and scroll down to see a **forever** block. Drag the
**forever** block beneath the **on start** block.

Go to **Loops**, drag and drop the **while(true)...do** block and drag out the
Comparison statement from **Logic**.

![](media/fc9c990ec8aceef0a75f5d0ddbad362e.png)
![](media/293cad1aa20498c434b8f082aa5e05fd.png)

In the forever block, we set to output 2 kinds of square wave of different
frequencies, so as to output a tone of corresponding frequency.

First, set a variable to output 80 pulse square wave, with a period of 2
milliseconds, and the frequency is 500 Hz, so the passive buzzer outputs 500 Hz
frequency sound for 160 milliseconds.

Then delay 100 milliseconds.

And set to output 100 pulse square wave, with a period of 4 milliseconds, and
the frequency is 250Hz, so the passive buzzer outputs 250Hz frequency sound for
400 milliseconds.

**Code 1:**

![](media/1a1af3e7f6a828da253704ea8c77275f.png)
...
Done sending the code 1 to micro:bit, the passive buzzer will alternately make
two sounds.
...
If you drag the source code we provided to the Microsoft MakeCode Block editor
[https://makecode.micro:bit.org/\#editor](https://makecode.microbit.org/#editor)

Click the icon
[![](https://wiki.keyestudio.com/File:361_7-6-8.png)
...
![](media/e3eb2b64d8e1336373a85d3cfd3f3f3e.jpeg)
...
![](media/39f18c8f863abbd91f81162d336433e0.jpeg)This breakout board has
been designed to offer an easy way to connect additional circuits and Components
to the edge connector on the BBC micro:bit.
...
-   Direct to connect the battery case carried with batteries to micro:bit main
    board for powering;

-   Connect the golden rings 3V GND with alligator clip lines for power supply;

![](media/9c7d340c158c65aba57271f6d23d2c63.jpeg)The breakout board also
comes with 4 fixed holes at the edge, easy to mount on any other devices.
...
![](media/ddcf94277862109b8ab13f6831fe7ca5.png)

**Coding**

So now let's move to coding. Let us see how to code the 2 LED modules to flash.
Below are some steps to follow.
...
Let's get started and code the 2 light modules flash on. To do so, you just need
to go to **Basic** and scroll down to see an **on start** block.
...
Go to the **Pins**, drag and drop the **digital write pin(P0) to(0)** block into
**forever** block and duplicate it several times.

![](media/2826010318a0ef4893800476078b7004.png)

Look at the connection diagram, **we separately connect the Red,Green, Blue pin
to P1, P2, P3. The white LED’s signal pin to P0.**

So we first set the white LED’s signal pin value to **1**, which means input a
**HIGH** level to **turn on** the white LED. Set to 0 to turn it off.

The RGB pins are active at **LOW (record 0)**, which means set to 0 to turn on;
so we can set the RGB pins to **HIGH (record 1)** to turn off.

If you want the LED flash as fast, add a **pause** block in millisecond, try
200ms.

Now we separately turn the white LED (P0) on for 1 second then off, followed by
turn Red LED (P1) on for 1 second then off; turn Green LED (P2) on for 1 second
then off; turn the Blue LED (P3) on for 1 second then off.
...
The white LED module will turn on then off, followed by RGB turning on red,
green and blue, with an interval of 200ms, alternately and circularly.

![](media/9f81d93161f64e67e47e4f3e3929578a.png)

## **Project 22: Use Button to Control Light**

**Overview**

In the above projects, we’ve introduced each sensor module. We now combine those
sensor modules to make interactive projects.

In this project, we would like to turn an LED on and off via a tactile button.

**![](media/c1955812294836b39b5683913085008a.jpeg)Components Required:**

-   Micro:bit main board \*1

-   Keyestudio Tactile Button Module for micro:bit \*1

-   ![](media/4978f5c5c05a016b1a2515dd4b23af3a.jpeg)keyestudio 1W LED
    Module for micro:bit \*1
...
Insert firmly the micro:bit main board into keyestudio Edge Connector IO
Breakout Board. Then connect the sensor module to micro:bit main board with
alligator clip lines.
...
![F:\\新建文件夹 (2)\\新建文件夹\\KS4056\\组合图源文件(课程
22).png组合图源文件(课程 22)](media/79d9ae65100bdf19ff95518b6b12feb8.png)

**Coding**

So now let's move to coding. Let us see how to code the LED module on with
tactile button module. Below are some steps to follow.
...
Let's get started and turn on LED via button control. To do so, you just need to
go to **Basic** and scroll down to see an **on start** block.

Now drag and drop, and go to **Led** and click **more** to drag out the block
**led enable(false)** into **on start** block.

![](media/4febcc7a71cca04e9c87395657a7b41b.png)
...
Drag this logic conditional block into **forever** block. And add a comparison
block to the logic conditional block.
...
Look back at the connection diagram, we connect the button’s signal pin to P0,
LED’s signal pin to P1. So if set the **P0** to **1**, means the button is
pressed, then set the **P1** to **1**, means input a **HIGH** level to **lit**
the LED. Or else, do the contrary action, release the button to turn the led
off.
...
Powered on, press the button, the LED on the module should turn on; release the
button, the LED turns off.

![](media/698ba951d082a9a0144a3046194a5df5.jpeg)
...
In this project, we are going to turn RGB LED alternately on red, green and blue
via capacitive touch control.

![](media/c30f6c28faf09945f6f3dff2ac75736f.jpeg)**Components Required:**

-   Micro:bit main board \*1

-   Keyestudio Capacitive Touch Module for micro:bit \*1

-   **![](media/df4998f32388d7e4f5ed39dadce17119.jpeg)**keyestudio 5050
    RGB Module for micro:bit \*1
...
Open the
[https://makecode.micro:bit.org/\#editor](https://makecode.microbit.org/#editor)
to write your code. Or can direct download and send the micro:bit.hex file to
your micro:bit.

![](media/e8126eb39256b743e77ed2cd02072a08.png)

**Code Explanation:**

Look back at the connection diagram, we have connected the touch module to
**P0**; **Red, Green, Blue LED to P1, P2, P3**.

The RGB pins are active at **LOW (record 0)**, which means set to 0 to turn on;
so we can set the RGB pins to **HIGH (record 1)** to turn off.

On start block, we initially set the variables **flag** and **count** block to
0;

![](media/44f9020067bccf54dd3b6a71486d512f.png)
![](media/e8126eb39256b743e77ed2cd02072a08.png)

For the forever block, when detects the touch signal, we set the **flag** to 1,
and add 10ms block for delay;
...
If **count** is greater than and equals to 3, then set the count to 0, run the
action that RGB light turns on red;
...
Tap the capacitive touch module with your finger, RGB will turn green; tap the
touch area, turn on blue; tap again, turn on red. It will form a loop.
...
In this project, you will learn how to trigger a buzzer alarm when detecting
someone moving nearby.

![](media/78ce8a388cc17b7a057b5fbcfd54ca42.jpeg)**Components Required:**
...
-   ![](media/088f490857d1e1944eaef873093def23.jpeg)Alligator clip cable
    \*6
...
So now let's move to coding. Let us see how to code the passive buzzer to alarm
when PIR motion module detects someone moving nearby.
...
Let's get started with triggering the buzzer sound. To do so, you just need to
go to **Basic** and scroll down to see an **on start** block.
...
Look back at the connection diagram, we connect the PIR motion sensor’s signal
pin to P1. So change the pin **P0** to **P1.**

Then go to **Music**, drag and drop the block **play tone(Middle C) for (1
beat)** and **rest(ms)(1 beat)**

![](media/bbfed2937384c24f17838ede79b1ce9b.png)

If the PIR motion sensor detects someone moving nearby, then the buzzer will
play a tone (Middle C); otherwise, buzzer turns off.
...
![](media/7b84e35f0d989ebb89d94dd999e32fa7.png)
...
When someone is moving nearby, the built-in led on PIR motion sensor lights and
buzzer will play a tone (Middle C). otherwise, the led is off and buzzer not
sounds.

![](media/f9e7cef6a0b2322dae09b91779fae698.jpeg)
...
![](media/78ce8a388cc17b7a057b5fbcfd54ca42.jpeg)

**Components Required:**

-   Micro:bit main board \*1

-   ![](media/c0801e7aeaeaff1a0c488e19f9a36680.jpeg)Keyestudio Passive
    Buzzer Module for micro:bit \*1

-   Keyestudio TEMT6000 Light Module for micro:bit \*1

-   Keyestudio Microphone Module for micro:bit \*1

-   ![](media/c24a8fe8646ec6459c216b6b45a214d1.jpeg)Alligator clip cable
    \*9
...
![](media/c920373b400bfbacd61a38f820d3d4cf.png)

**Coding**

So now let's move to coding. Let us see how we can code buzzer sound according
to the analog value of both TEMT6000 light module and microphone module. Below
are some steps to follow.
...
Let's get started with playing tones. To do so, you just need to go to **Basic**
and scroll down to see an **on start** block.
...
Next, go to **Serial**, drag and drop the block **serial write value(x)=(0)**
into the **forever** block and duplicate once.

In this way it can write both brightness and sound value to the serial port and
show it on monitor.
...
And drag out the block **analog read pin(P0)** from **Pins** to replace the
“**0**” field.

According to the connection, microphone module’s signal pin is connected to P2;
light module’s signal pin is connected to P1.
...
Supposed that both light and MIC analog value are smaller than or equal to 100,
buzzer not beeps. How should we code?

First drag the **while(true)...do** block out from the **Loops**. Then add the
Boolean and Comparison logic statements.
...
Then drag this block into the **while(true)...do** block, replacing the
“**true**” field. And again duplicate the block **analog read pin(P1)** and the
block **analog read pin(P2)** and drag them into the first input. Set both
values smaller than and equal to 100.

While the condition is met, run the action that buzzer not beeps. We can go to
drag the **rest(ms)(1 beat)** from **Music** into the while block just made.

![](media/39cd809bea097b1e7f89587c67dd8387.png)

Now we’ve set both light and MIC analog value are smaller than or equal to 100,
buzzer not beeps. As shown below.

![](media/2dcadc6a3c6e6a0f5c6e71d12d0300a2.png)

Furthermore, if we want to add several condition statements, be able to
duplicate the **while...do...** block several times, and set to different input
values to trigger buzzer ring different tones.

Have your try! Write the conditions as follows:

-   When the light analog value is greater than 100, and MIC analog value is
    smaller than or equal to 100, the buzzer will play a tone (Low B);

-   When the light analog value is smaller than or equal to 100, and MIC analog
    value is greater than 100, the buzzer will play a tone (Middle D);

-   When both light and MIC analog value are greater than 100, the buzzer play a
    tone (High E).
...
Done uploading the code, it measures the analog value of ambient light and
microphone sound, which can be applied to make a buzzer sound.

When both light and MIC analog value are less than or equal to 100, buzzer not
beeps;

When the light analog value is greater than 100, and MIC analog value is less
than or equal to 100, the buzzer will play a tone (Low B);

When the light analog value is less than or equal to 100, and MIC analog value
is greater than 100, the buzzer will play a tone (Middle D);

When both light and MIC analog value are greater than 100, the buzzer play a
tone (High E).

![](media/3e7dd99c256d85dae3ebe9a7c4622027.jpeg)
...
In this project, you will learn how to trigger the RGB light shining colors
based on the measured analog value of ambient light.

![](media/c0801e7aeaeaff1a0c488e19f9a36680.jpeg)**Components Required:**
...
![](media/21b5c462208691ba13bf604fa5404366.png)

**Coding**

So now let's move to coding. Let us see how we can code the RGB flash on
different colors in different analog value of ambient light.
...
Let's get started with controlling RGB colors with light sensor. To do so, you
just need to go to **Basic** and scroll down to see an **on start** block.
...
Then go to **Variables**, search for **set (val) to (0)** block and drag this
block into **forever** block.

![](media/9c58e8bbf42e5948358851b05fdae85c.png)

Go to **Pins**, drag and drop the block **analog read pin(P0)** into **set (val)
to (0)** block, replacing the “**0**” field.

Look back at the connection diagram, we have connected the TEMT6000 light module
to P0; **Red, Green, Blue LED to P1, P2, P3**.

The RGB pins are active at **LOW (record 0)**, which means set to 0 to turn on;
so we can set the RGB pins to **HIGH (record 1)** to turn off.

Now we set to print the light value to the serial port using the **serial write
number(0)** block from **Serial** and set the logic conditionals.

![](media/35808c4265e9f7f15728eb45b77af2f7.png)

Drag the **val** block from the **Variables** and drop into **serial write
number(0),** replacing the “**0**” field.

Then add the logic conditionals block, we go to use light value to turn the RGB
LED on.
...
In other word, go to drag and duplicate **digital write pin(P0) to (0)** block
twice. For RGB, separately **set Pin1 to 0 (LOW, turn on); Pin2, Pin3 to 1
(HIHG, turn off)**

![](media/a9480041c2e9722a31f93d135a7b7ce5.png)

Furthermore, you can follow the same method to turn on the Green LED or Blue
LED.
...
![](media/f1597e044e8d9d629dcce8cbacddb45f.png)
...
When analog value of ambient light is less than or equal to 300, RGB light turns
on red; when analog value is greater than 300 and less than 800, RGB light turns
on green; when analog value is greater than or equal to 800, RGB light turns on
blue;

![](media/8db21f8967483122a9aabe0639a27483.jpeg)
...
In this project, you will learn how to make a buzzer play different tones with a
tactile button.
...
-   **![](media/c1955812294836b39b5683913085008a.jpeg)**Keyestudio
    Tactile Button Module for micro:bit \*1
